...
class RealizedPnLGenerator:
    """
    已實現損益資料產生器
    負責產生已實現損益(已賣出)資料並轉換為 CSV 格式
    自動計算: 已實現損益, 報酬率%
    """
    ...
    def get_user_input(self):
        ...
        while True:
            print(f"\n--- 新增一筆交易 (預設日期: {default_date}) ---")
            
            # 1. 輸入日期 (嚴格驗證 YYYYMMDD)
            date_str = ""
            while True:
                date_input = input(f"📅 請輸入交易日期 (YYYYMMDD) [預設 {default_date}]: ").strip()
            ...
            for idx, s in enumerate(self.stocks, 1):
                market_aligned = self._align_text(f"[{s['市場']}]", 13)
            ...
            shares = 0
            while True:
                shares_input = input(f"🔢 請輸入 [{name}] 賣出股數: ").strip()
            ...
            cost = 0
            while True:
                cost_input = input(f"💰 請輸入 [{name}] 總成本 (這些股份的買入成本): ").strip()
                if cost_input.lower() in ['q', 'exit']:
                    if data: return data
                    return []
                try:
                    cost = int(float(cost_input))
                    if cost < 0:
                        print("❌ 總成本不能為負數")
                        continue
                    break
            ...
            sold_price = 0
            while True:
                price_input = input(f"💵 請輸入 [{name}] 賣出總價 (入帳金額): ").strip()
                if price_input.lower() in ['q', 'exit']:
                    if data: return data
                    return []
                try:
                    sold_price = int(float(price_input))
                    break
                except ValueError:
                    print("❌ 金額必須為數字")

            # 6. 自動計算
            realized_pnl = sold_price - cost
            roi = 0.0
            if cost != 0:
                roi = (realized_pnl / cost) * 100
            ...
            print(f"✅ 已暫存: {name} | 損益: {pnl_color} ${realized_pnl:,} ({roi:.2f}%)")
    ...
    def generate_csv(self, data, output_path):
        ...
        fieldnames = ["日期", "市場", "股票代號", "股票名稱", "賣出股數", "總成本", "賣出價", "已實現損益", "報酬率%"]
...
if __name__ == "__main__":
    generator = RealizedPnLGenerator()
