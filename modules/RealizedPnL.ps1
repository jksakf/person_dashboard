...
function Invoke-RealizedPnLFlow {
    Write-Log ">>> 正在啟動 [已實現損益] 模組..." -Level Info
    ...
    try {
        while ($true) {
            Write-Host "`n--- 新增一筆已實現損益 (預設日期: $defaultDate) ---" -ForegroundColor Green
            ...
            # 3. 賣出股數
            $sharesStr = Get-CleanInput -Prompt "賣出股數"
            ...
            # 5. 賣出價 (總賣出金額)
            $sellPriceStr = Get-CleanInput -Prompt "賣出價 (總賣出金額)"
            try { $sellPrice = [decimal]$sellPriceStr } catch { Write-Log "❌ 金額格式錯誤" -Level Warning; continue }

            # 計算邏輯：已實現損益 = 賣出價 - 總成本
            $pnl = $sellPrice - $totalCost
            ...
            Write-Log "✅ 已暫存: $inputName | 損益: $([int]$pnl) ($roiStr)" -Level Info
        }
    }
    catch {
